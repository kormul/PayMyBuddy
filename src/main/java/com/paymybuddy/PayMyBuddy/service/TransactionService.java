...
public class TransactionService {
	...
	public boolean addTransaction(double amount, String description, int receiver) {
		...
		transaction.setTransmitter(userTransmitter);
	...
	public List<TransactionHistoryDTO> getPageTransaction(int page) {
		...
		for(int i = 0; i<3;i++) {
			if(id+i<allTransaction.size()) {
				...
				historyResult.add(new TransactionHistoryDTO(allTransaction.get(id+i).getReceiver().getPseudo(), allTransaction.get(id+i).getDescription(), allTransaction.get(id+i).getAmount()));
			}
			else {
				break;
			}
